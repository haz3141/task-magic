...
import { ActorClient, ACTOR_STORAGE_KEY, generateActorId } from "./actor";
...
interface UseActorResult {
    ...
    isLoading: boolean;
    needsSetup: boolean;
    createActor: (name: string, emoji: string) => void;
}
...
export function useActor(): UseActorResult {
    ...
    useEffect(() => {
        try {
            ...
            if (stored) {
                const parsed: Actor = JSON.parse(stored);
                setActor({
                    id: parsed.id,
                    name: parsed.name,
                    emoji: parsed.emoji,
    ...
    const createActor = useCallback((name: string, emoji: string) => {
